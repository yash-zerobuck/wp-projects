 ...
 */
namespace ElementskitVendor\Google;

use ElementskitVendor\Google\AccessToken\Revoke;
use ElementskitVendor\Google\AccessToken\Verify;
use ElementskitVendor\Google\Auth\ApplicationDefaultCredentials;
use ElementskitVendor\Google\Auth\Cache\MemoryCacheItemPool;
use ElementskitVendor\Google\Auth\CredentialsLoader;
use ElementskitVendor\Google\Auth\FetchAuthTokenCache;
use ElementskitVendor\Google\Auth\HttpHandler\HttpHandlerFactory;
use ElementskitVendor\Google\Auth\OAuth2;
use ElementskitVendor\Google\Auth\Credentials\ServiceAccountCredentials;
use ElementskitVendor\Google\Auth\Credentials\UserRefreshCredentials;
use ElementskitVendor\Google\AuthHandler\AuthHandlerFactory;
use ElementskitVendor\Google\Http\REST;
use ElementskitVendor\GuzzleHttp\Client as GuzzleClient;
use ElementskitVendor\GuzzleHttp\ClientInterface;
use ElementskitVendor\GuzzleHttp\Ring\Client\StreamHandler;
use ElementskitVendor\Psr\Cache\CacheItemPoolInterface;
use ElementskitVendor\Psr\Http\Message\RequestInterface;
use ElementskitVendor\Psr\Log\LoggerInterface;
use ElementskitVendor\Monolog\Logger;
use ElementskitVendor\Monolog\Handler\StreamHandler as MonologStreamHandler;
use ElementskitVendor\Monolog\Handler\SyslogHandler as MonologSyslogHandler;
...
class Client
{
    ...
    public function setAuthConfig($config)
    {
        ...
        if (isset($config['type']) && $config['type'] == 'service_account') {
            ...
            // new-style
            isset($config['client_id']) && $this->setClientId($config['client_id']);
            isset($config['client_secret']) && $this->setClientSecret($config['client_secret']);
            isset($config['redirect_uris']) && $this->setRedirectUri($config['redirect_uris'][0]);
        }
    }
    /**
    ...
    {
        $guzzleVersion = null;
        if (\defined('\\ElementskitVendor\\GuzzleHttp\\ClientInterface::MAJOR_VERSION')) {
            $guzzleVersion = ClientInterface::MAJOR_VERSION;
        } elseif (\defined('\\ElementskitVendor\\GuzzleHttp\\ClientInterface::VERSION')) {
